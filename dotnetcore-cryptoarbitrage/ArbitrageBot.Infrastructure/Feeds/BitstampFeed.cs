...
/// <summary>
/// Feed WebSocket + REST fallback de Bitstamp.
/// WebSocket: order_book_{symbol} canal via bts:subscribe.
/// REST: order_book limit=1 como fallback cada 1.5s.
/// </summary>
public sealed class BitstampFeed : IExchangeFeed, IDisposable
{
    private readonly ILogger<BitstampFeed> _logger;
    ...
    public string ExchangeId => "Bitstamp";
    public event Action<OrderBook>? OnOrderBookUpdated;

    public BitstampFeed(
        ILogger<BitstampFeed> logger,
        ...
        IOptions<ExchangeOptions> options)
    {
        ...
        _health = health;
        var cfg = options.Value.Exchanges["Bitstamp"];
    ...
    private async Task ConnectLoopAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                var subscribeMsg = $"{{\"event\":\"bts:subscribe\"," +
                    $"\"data\":{{\"channel\":\"order_book_{_symbol}\"}}}}";
    ...
    private OrderBook? ParseOrderBook(string raw)
    {
        try
        {
            ...
            var bids = data["bids"]?.First;
            var asks = data["asks"]?.First;
