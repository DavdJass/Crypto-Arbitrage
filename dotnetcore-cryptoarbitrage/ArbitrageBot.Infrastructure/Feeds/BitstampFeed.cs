...
public sealed class BitstampFeed : IExchangeFeed, IDisposable
{
    private readonly ILogger<BitstampFeed> _l;private readonly HttpClient _hc;private readonly FeedHealthTracker _h;private readonly string _ws,_rest,_symbol;private ClientWebSocket? _wsCli;private CancellationTokenSource? _cts;private bool _d,_fallback;
    public string ExchangeId=>"Bitstamp";public event Action<OrderBook>? OnOrderBookUpdated;
    public BitstampFeed(ILogger<BitstampFeed> l,HttpClient hc,FeedHealthTracker h,IOptions<ExchangeOptions> o){_l=l;_hc=hc;_h=h;var c=o.Value.Exchanges["Bitstamp"];_ws=c.WebSocketUrl;_rest=c.RestUrl;_symbol=c.Symbol;}
    ...
    OrderBook? Parse(string raw){try{var j=JObject.Parse(raw);var d=j["data"];if(d==null)return null;var bids=d["bids"]?.First;var asks=d["asks"]?.First;if(bids==null||asks==null)return null;return new(ExchangeId,(decimal)asks[0],(decimal)bids[0],(decimal)(bids[1]??0),(decimal)(asks[1]??0),DateTime.UtcNow);}catch{return null;}}
    OrderBook? ParseRest(string raw){try{var j=JObject.Parse(raw);var bids=j["bids"]?.First;var asks=j["asks"]?.First;if(bids==null||asks==null)return null;return new(ExchangeId,(decimal)asks[0],(decimal)bids[0],(decimal)(bids[1]??0),(decimal)(asks[1]??0),DateTime.UtcNow);}catch{return null;}}
