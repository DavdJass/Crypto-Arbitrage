...
public sealed class BitstampFeed : IExchangeFeed, IDisposable
{
    ...
    async Task ConnectLoop(CancellationToken ct){var d=2;while(!ct.IsCancellationRequested){try{_wsCli?.Dispose();_wsCli=new();_wsCli.Options.KeepAliveInterval=TimeSpan.FromSeconds(30);await _wsCli.ConnectAsync(new(_ws),ct);var sub=$"{{\"event\":\"bts:subscribe\",\"data\":{{\"channel\":\"order_book_{_symbol}\"}}}}";await _wsCli.SendAsync(Encoding.UTF8.GetBytes(sub),WebSocketMessageType.Text,true,ct);_h.SetStatus(ExchangeId,"connected","WS");_fallback=false;_l.LogInformation("[{X}] WS conectado",ExchangeId);d=2;await ReceiveLoop(ct);}catch(OperationCanceledException){break;}catch(Exception ex){_l.LogError(ex,"[{X}] WS error",ExchangeId);_fallback=true;await RestLoop(ct);await Task.Delay(TimeSpan.FromSeconds(Math.Min(d,30)),ct);d*=2;}}}
    ...
    OrderBook? Parse(string raw){try{var j=JObject.Parse(raw);var d=j["data"];if(d==null)return null;var bids=d["bids"]?.First;var asks=d["asks"]?.First;if(bids==null||asks==null)return null;return new(ExchangeId,(decimal)bids[0],(decimal)asks[0],(decimal)(bids[1]??0),(decimal)(asks[1]??0),DateTime.UtcNow);}catch{return null;}}
