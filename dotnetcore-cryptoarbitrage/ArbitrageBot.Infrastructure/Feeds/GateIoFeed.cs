...
public sealed class GateIoFeed : IExchangeFeed, IDisposable
{
    ...
    async Task ConnectLoop(CancellationToken ct){var d=2;while(!ct.IsCancellationRequested){try{_wsCli?.Dispose();_wsCli=new();await _wsCli.ConnectAsync(new(_ws),ct);var sub=$"{{\"time\":{DateTimeOffset.UtcNow.ToUnixTimeSeconds()},\"channel\":\"spot.order_book\",\"event\":\"subscribe\",\"payload\":[\"{_symbol}\",\"1\",\"100ms\"]}}";
    ...
    OrderBook? Parse(string raw){try{var j=JObject.Parse(raw);var r=j["result"];if(r==null)return null;var bids=r["bids"]?.First;var asks=r["asks"]?.First;if(bids==null||asks==null)return null;return new(ExchangeId,(decimal)asks[0],(decimal)bids[0],(decimal)(bids[1]??0),(decimal)(asks[1]??0),DateTime.UtcNow);}catch{return null;}}
